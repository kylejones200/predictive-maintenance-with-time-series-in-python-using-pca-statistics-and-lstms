...
def main():
    parser = argparse.ArgumentParser(
        description="Predictive Maintenance with PCA, Statistics, and LSTMs"
    )
    ...
    df_model = df[
        ["unit_number", "time_in_cycles", "RUL"] + config["model"]["sensor_columns"]
    ]

    if config["model"]["exponential_regression"]:
        glm_results, X = fit_exponential_regression(
            df_model, config["model"]["sensor_columns"]
        )
