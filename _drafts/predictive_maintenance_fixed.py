...
def load_fd001():
    try:
        ...
        raise FileNotFoundError(
            "NASA CMAPSS FD001 files not found. See DATA_NASA_CMAPSS.md for instructions."
        ) from e
    rul.columns = ["max_RUL"]
    rul["unit"] = rul.index + 1
    last_cycle = (
        df.groupby("unit")["time"]
        .max()
        .reset_index()
        .rename(columns={"time": "max_time"})
    )
...
def make_rf_sequences(df, sensor, window=30):
    ...
    for uid, g in df.groupby("unit"):
        vals = g[sensor].values
        target = g["distress"].values
...
def main(plot: bool = False):
    ...
    logger.info(
        "RF mean metrics:",
        {k: np.nanmean([m[k] for m in rf_results]) for k in rf_results[0]},
    )
    ...
    for tr, te in unit_time_series_splits(u, n_splits=5):
        ...
        Xtr_seq = pca.transform(Xtr_s.reshape(-1, n_features)).reshape(
            Xtr.shape[0], Xtr.shape[1], 3
        )
        Xte_seq = pca.transform(Xte_s.reshape(-1, n_features)).reshape(
            Xte.shape[0], Xte.shape[1], 3
        )
        model = Sequential(
            [
                LSTM(64, input_shape=(Xtr_seq.shape[1], Xtr_seq.shape[2])),
    ...
    logger.info(
        "LSTM mean metrics:",
        {k: np.nanmean([m[k] for m in lstm_results]) for k in lstm_results[0]},
    )

    # Engine plot from last fold transforms
    if last_fold is not None:
        scaler, pca, model = last_fold
        unit_id = int(df["unit"].iloc[0])
        window = 30
        eng = df[df["unit"] == unit_id].sort_values("time")
        raw = np.array(
            [
                eng[selected_sensors].values[i : i + window]
        ...
        if len(raw):
            ...
            seq = pca.transform(raw_s.reshape(-1, n_features)).reshape(
    ...
    if plot:
        ...
        plt.plot(cycles, pred, label="LSTM")
        plt.axhline(0.5, color="red", linestyle=":", linewidth=0.8)
        plt.title(f"Engine {unit_id} – LSTM Distress Probability")
        plt.xlabel("Cycle")
        plt.ylabel("Probability")
        plt.legend()
        signalplot.save("pm_lstm_engine.png")
